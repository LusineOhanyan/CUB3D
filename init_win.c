...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/05/11 16:16:15 by lohanyan          #+#    #+#             */
/*   Updated: 2023/05/18 15:26:25 by lohanyan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	find_pos_player(t_addres *address)
{
	...
	i = 0;
	while (address->vars.map[i])
	{
		j = 0;
		while (address->vars.map[i][j])
		{
			if (address->vars.map[i][j] == 'N' || \
				address->vars.map[i][j] == 'S' || \
				address->vars.map[i][j] == 'E' || \
				address->vars.map[i][j] == 'W')
			{
				init_player_pos(address, i, j);
				address->rcasting->posx = i + 0.5;
				address->rcasting->posy = j + 0.5;
				address->vars.map[i][j] = '0';
				return ;
			}
			j++;
		}
...
void	norm_init_win_2(t_addres *address)
{
	...
	while (address->rcasting->hit == 0)
	{
		...
		if (address->vars.map[address->rcasting->mapx] \
